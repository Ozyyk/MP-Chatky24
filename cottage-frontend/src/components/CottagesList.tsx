...
export default function CottageList() {
  ...
  const fetchCottages = async () => {
    try {
      const res = await fetch('https://belohrad.jarda.site/api/cottages'); // Fetching all cottages
      if (res.ok) {
        ...
        const cottagesWithRatings = await Promise.all(
          data.map(async (cottage: Cottage) => {
            const ratingRes = await fetch(`https://belohrad.jarda.site/api/comments/${cottage._id}`);
            if (ratingRes.ok) {
            ...
            return { ...cottage, averageRating: 0 };
          })
        );
